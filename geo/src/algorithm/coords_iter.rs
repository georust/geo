...
pub trait CoordsIter {
    ...
    /// ```
    fn coords_iter(&self) -> impl Iterator<Item = Coord<Self::Scalar>>;
    ...
    /// assert_eq!(None, iter.next());
    /// ```
    fn exterior_coords_iter(&self) -> impl Iterator<Item = Coord<Self::Scalar>>;
}
...
impl<T: CoordNum> CoordsIter for Point<T> {
    ...
    fn coords_iter(&self) -> impl Iterator<Item = Coord<T>> {
        iter::once(self.0)
    }
    ...
    fn exterior_coords_iter(&self) -> impl Iterator<Item = Coord<T>> {
        self.coords_iter()
    }
}
...
impl<T: CoordNum> CoordsIter for Polygon<T> {
    ...
    fn coords_iter(&self) -> impl Iterator<Item = Coord<T>> {
        self.exterior()
            .coords_iter()
            .chain(self.interiors().iter().flat_map(|p| p.coords_iter()))
    }
...
impl<T: CoordNum> CoordsIter for MultiPoint<T> {
    ...
    fn coords_iter(&self) -> impl Iterator<Item = Coord<T>> {
        self.0.iter().flat_map(|p| p.coords_iter())
    }
...
impl<T: CoordNum> CoordsIter for MultiLineString<T> {
    ...
    fn coords_iter(&self) -> impl Iterator<Item = Coord<T>> {
        self.0.iter().flat_map(|l| l.coords_iter())
    }
...
impl<T: CoordNum> CoordsIter for MultiPolygon<T> {
    ...
    fn exterior_coords_iter(&self) -> impl Iterator<Item = Coord<T>> {
        self.0.iter().flat_map(|p| p.exterior_coords_iter())
    }
}
...
impl<T: CoordNum> CoordsIter for Geometry<T> {
    ...
    fn coords_iter(&self) -> impl Iterator<Item = Coord<T>> {
        match self {
            Geometry::Point(g) => Box::new(g.coords_iter()) as Box<dyn Iterator<Item = Coord<T>>>,
            ...
            Geometry::MultiPolygon(g) => Box::new(g.coords_iter()),
    ...
    fn exterior_coords_iter(&self) -> impl Iterator<Item = Coord<T>> {
        match self {
            Geometry::Point(g) => {
                Box::new(g.exterior_coords_iter()) as Box<dyn Iterator<Item = Coord<T>>>
            }
            Geometry::Line(g) => Box::new(g.exterior_coords_iter()),
            Geometry::LineString(g) => Box::new(g.exterior_coords_iter()),
            Geometry::Polygon(g) => Box::new(g.exterior_coords_iter()),
            Geometry::MultiPoint(g) => Box::new(g.exterior_coords_iter()),
            Geometry::MultiLineString(g) => Box::new(g.exterior_coords_iter()),
            Geometry::MultiPolygon(g) => Box::new(g.exterior_coords_iter()),
            Geometry::GeometryCollection(g) => Box::new(g.exterior_coords_iter()),
            Geometry::Rect(g) => Box::new(g.exterior_coords_iter()),
