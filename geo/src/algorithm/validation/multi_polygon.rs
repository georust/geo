...
pub enum InvalidMultiPolygon {
    /// For a [`MultiPolygon`] to be valid, each member [`Polygon`] must be valid.
    InvalidPolygon(GeometryIndex, InvalidPolygon),
    /// No [`Polygon`] in a valid [`MultiPolygon`] may overlap (2-dimensional intersection)
    ElementsOverlaps(GeometryIndex, GeometryIndex),
    /// No [`Polygon`] in a valid [`MultiPolygon`] may touch on a line (1-dimensional intersection)
