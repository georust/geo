    ...
    T: Float + FloatConst + Signed,
{
    ...
    let ymin1 = Point(poly1.exterior[poly1_extremes.ymin]);
    let ymax2 = Point(poly2.exterior[poly2_extremes.ymax]);
...
where
    T: Float,
{
    (current_vertex + (poly.exterior.len() - 1) - 1) % (poly.exterior.len() - 1)
}
...
where
    T: Float,
{
    (current_vertex + 1) % (poly.exterior.len() - 1)
}
    ...
    T: Float + FloatConst + Signed,
{
    ...
    if (state.ap1 - minangle).abs() < T::from(0.002).unwrap() {
        ...
        state.p1next = Point(state.poly1.exterior[p1next]);
        state.p1_idx = p1next;
    ...
    if (state.aq2 - minangle).abs() < T::from(0.002).unwrap() {
        ...
        state.q2next = Point(state.poly2.exterior[q2next]);
        state.q2_idx = q2next;
