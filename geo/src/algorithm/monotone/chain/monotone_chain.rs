use super::MonotoneChainSegmentFactory;
use super::monotone_segment::MonotoneChainSegment;
...
impl<'a, T: GeoNum> MonotoneChain<'a, T> {
    pub(crate) fn segment_iter(&self) -> impl Iterator<Item = &MonotoneChainSegment<'a, T>> {
        self.segments.iter()
    }
}
