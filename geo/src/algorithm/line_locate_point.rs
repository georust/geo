use crate::prelude::*;
use crate::{Closest, LineString, MultiLineString, Point, Coordinate, Line};
...
{
    fn locate_point(&self, p: &Point<F>) -> Option<F> {
        let distance_along_line: F = self.lines().map(
            |line| line.euclidean_length() * line.locate_point(p).unwrap()
        ).sum();
