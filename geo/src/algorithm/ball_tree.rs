...
impl<T: GeoFloat, D> BallTree<T, D> {
    ...
    /// [`NodeKind::Branch`] are also valid.
    ///
    /// # Panics
    ///
    /// Panics if `idx >= node_count()` (in particular, for any `idx` on an
    /// empty tree).
...
mod tests {
    ...
    fn test_min_distance_to_overlapping_balls_is_zero() {
        ...
        let NodeKind::Branch { left, right } = root.kind() else {
            panic!("fixture tree must have a branch root");
        };
