use crate::{
    algorithm::{centroid::Centroid, rotate::Rotate, BoundingRect},
    Area, ConvexHull, CoordFloat, GeoFloat, GeoNum, Point, Polygon,
};
/// Returns the minimun bounding rect of polygon.
///

pub trait Mbr<T: CoordFloat> {
    type Output: Into<Option<Polygon<T>>>;
    ...
    /// );
    /// ```
    fn mbr(&self) -> Self::Output;
}

impl<T> Mbr<T> for Polygon<T>
where
    T: CoordFloat,
    T: GeoFloat,
    T: GeoNum,
    f64: From<T>,
{
    ...
    fn mbr(&self) -> Self::Output {
        ...
        let points = self.exterior().clone().into_points();
