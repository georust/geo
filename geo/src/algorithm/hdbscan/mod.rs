...
fn find_clusters<T: GeoFloat>(
    ...
    // The reference python `hdbscan` library excludes the root from
    // selection unless its `allow_single_cluster` parameter (default
    // `False`) is set; this implementation matches the default and does not
    // expose the option. Points that reach the root without passing through
    // a selected cluster are labelled noise. A complete MST
