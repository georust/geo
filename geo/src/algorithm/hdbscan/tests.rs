...
/// Test boruvka_mst on the 7-point dataset from
...
///
/// Dendrogram for 7 points (adapted from
/// [petal-clustering](https://github.com/petabi/petal-clustering)'s condense_mst
/// test):
///
///             node 12
///           /          \           distance = 8.0
...
fn compute_core_data_retains_knn_indices() {
    ...
    let pts: Vec<Point<f64>> = wkt!(MULTIPOINT(
...
fn seed_best_edges_seeds_tightest_cross_component_mrd() {
    let pts: Vec<Point<f64>> = wkt!(MULTIPOINT(0.0 0.0, 1.0 0.0, 10.0 0.0, 11.0 0.0)).0;
    let tree = BallTree::new(pts.iter().copied());
