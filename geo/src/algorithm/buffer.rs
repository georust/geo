//! Buffering operations for geometric objects.
//!
//! Buffering creates a polygon that represents all points within a specified distance from the input geometry.
//! The resulting buffer can be styled with different line caps and joins to control the appearance of the buffered geometry.
...
use crate::bool_ops::i_overlay_integration::convert::{
    line_string_to_shape_path, ring_to_shape_path,
};
use crate::bool_ops::i_overlay_integration::BoolOpsCoord;
use crate::bool_ops::unary_union;
...
///
/// The buffer operation creates polygons that represents all points within a specified distance from the input geometry.
...
/// ```
/// use geo::{Point, Buffer};
///
/// let point = Point::new(0.0, 0.0);
/// let buffered = point.buffer(2.0); // Creates an approximated circle with radius 2.0
/// ```
///
...
pub trait Buffer {
    ...
    /// Creates a buffer around the geometry with the specified distance using default rounded styling.
    ///
    /// # Arguments
    ///
    ...
    /// let circle = point.buffer(3.0);
    ///
    /// use geo::algorithm::Area;
    /// // Creates an approximately circular polygon centered at (5, 5) with radius 30
    /// approx::assert_relative_eq!(circle.unsigned_area(), std::f64::consts::PI * 3.0f64.powi(2), epsilon = 2e-1);
    /// ```
    ...
    /// Creates a buffer around the geometry using the specified styling options.
    ///
...
impl<T: BoolOpsNum> BufferStyle<T> {
    ...
    fn stroke_style(&self) -> StrokeStyle<BoolOpsCoord<T>, T> {
        // Stroke is terms of "line width" but buffer is "distance from edge" so we double the line width.
        let two = T::one() + T::one();
...
impl<F: BoolOpsNum + 'static> Buffer for Point<F> {
    ...
    fn buffer_with_style(&self, style: BufferStyle<Self::Scalar>) -> MultiPolygon<Self::Scalar> {
        ...
        let num_segments = (2.0 * std::f64::consts::PI
            / BufferStyle::<F>::default_join_angle().to_f64())
        .ceil() as usize;
        let center = self.0;
        ...
        for i in 0..num_segments {
            let angle = F::from_f64(2.0 * std::f64::consts::PI / num_segments as f64 * i as f64)
...
mod tests {
    ...
    fn buffer_multi_polygon_elements_stay_separate() {
        ...
        let expected_output_from_jts = wkt! { MULTIPOLYGON (
    ...
    fn buffer_rect() {
        ...
        let expected_output_from_jts = wkt! {
            POLYGON ((-2. 0., -2. 5., -1.9615705608064609 5.390180644032257, -1.8477590650225735 5.76536686473018, -1.6629392246050907 6.111140466039204, -1.414213562373095 6.414213562373095, -1.111140466039204 6.662939224605091, -0.7653668647301795 6.847759065022574, -0.3901806440322564 6.961570560806461, 0. 7., 10. 7., 10.390180644032256 6.961570560806461, 10.76536686473018 6.847759065022574, 11.111140466039204 6.662939224605091, 11.414213562373096 6.414213562373095, 11.66293922460509 6.111140466039204, 11.847759065022574 5.765366864730179, 11.96157056080646 5.390180644032257, 12. 5., 12. 0., 11.96157056080646 -0.3901806440322565, 11.847759065022574 -0.7653668647301796, 11.66293922460509 -1.1111404660392044, 11.414213562373096 -1.414213562373095, 11.111140466039204 -1.6629392246050905, 10.76536686473018 -1.8477590650225735, 10.390180644032256 -1.9615705608064609, 10. -2., 0. -2., -0.3901806440322564 -1.9615705608064609, -0.7653668647301795 -1.8477590650225735, -1.111140466039204 -1.6629392246050907, -1.414213562373095 -1.4142135623730951, -1.6629392246050907 -1.1111404660392044, -1.8477590650225735 -0.7653668647301798, -1.9615705608064609 -0.3901806440322572, -2. 0.))
        };
        check_buffer_test_case(&actual.into(), &expected_output_from_jts.into()).unwrap();
    }
    ...
    fn buffer_multi_point_single_point() {
        ...
        let single_point = Point::new(2.0, 3.0);
        let expected = single_point.buffer(2.0);

        check_buffer_test_case(&actual.into(), &expected.into()).unwrap();
    }
    ...
    fn buffer_geometry_collection_separate_geometries() {
        ...
        let actual = geometry_collection.buffer(1.0);
    ...
    fn buffer_geometry_collection_overlapping_geometries() {
        ...
        // Should result in one merged polygon since buffers overlap
        assert_eq!(actual.0.len(), 1);
    }
