...
impl<'r, 'a, 'b, F: GeoFloat> TopologyComputer<'r, 'a, 'b, F> {
    ...
    pub fn is_area_area(&self) -> bool {
        self.dimension(InputIndex::A) == Dimensions::TwoDimensional
            && self.dimension(InputIndex::B) == Dimensions::TwoDimensional
    }
    ...
    pub fn add_point_on_geometry(
        ...
        if self.geometry_is_empty(other(source)) {
            return;
        }
