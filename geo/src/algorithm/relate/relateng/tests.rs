...
fn eval_pred(
    mut predicate: impl TopologyPredicate<f64>,
    wkta: &str,
    wktb: &str,
) -> (bool, &'static str) {
    let a = read(wkta);
    ...
    (relate_ng::eval(&ca, &cb, &mut predicate), name)
}

fn check_pred(predicate: impl TopologyPredicate<f64>, wkta: &str, wktb: &str, expected: bool) {
    let (actual, name) = eval_pred(predicate, wkta, wktb);
    assert_eq!(actual, expected, "{name}: A = {wkta}, B = {wktb}");
}
...
fn check_relate(wkta: &str, wktb: &str, expected: &str) {
    ...
    assert_eq!(
        format!("{im:?}"),
        format!("IntersectionMatrix({expected})"),
        "A = {wkta}, B = {wktb}"
    );
}
