//! Extract the section of a [`Line`] or [`LineString`] between two
//! distance-along-line (or ratio-along-line) values. A substring of a
//! `Line` is returned as a `Line`; a substring of a `LineString` is
...
pub trait SubstringableLine<F: CoordFloat> {
    /// The output type of substring extraction. Each impl returns the
    /// most precise geometry type available: the `Line` impl returns
    /// `Result<Line<F>, SubstringError>`, and the `LineString` impl
    /// returns `Result<LineString<F>, SubstringError>`.
    type Output;
...
impl<F: CoordFloat> SubstringableLine<F> for Line<F> {
    type Output = Result<Line<F>, SubstringError>;
...
mod tests {
    ...
    fn substring_line_full_range() {
        ...
        let expected = wkt!(LINE(0.0 0.0,10.0 0.0));
    ...
    fn substring_line_inner_range() {
        ...
        let expected = wkt!(LINE(2.0 0.0,7.0 0.0));
        assert_eq!(result, expected);
    }
    ...
    fn substring_line_zero_length_range() {
        ...
        let expected = wkt!(LINE(4.0 0.0,4.0 0.0));
        assert_eq!(result, expected);
    }
