...
impl<F: GeoFloat> Distance<F, &LineString<F>, &LineString<F>> for Euclidean {
    fn distance(&self, line_string_a: &LineString<F>, line_string_b: &LineString<F>) -> F {
        ...
        let bbox_a = line_string_a.bounding_rect();
        let bbox_b = line_string_b.bounding_rect();
        ...
        if let (Some(rect_a), Some(rect_b)) = (bbox_a, bbox_b) {
            ...
            if x_separated || y_separated {
                return distance_polygon_polygon_fast(line_string_a, line_string_b);
            }
        }
...
impl<F: GeoFloat> Distance<F, &LineString<F>, &Polygon<F>> for Euclidean {
    fn distance(&self, line_string: &LineString<F>, polygon: &Polygon<F>) -> F {
        ...
        // Check if bounding boxes are non-overlapping: if so, use project-and-prune optimisation
        let bbox_ls = line_string.bounding_rect();
        let bbox_poly = polygon.bounding_rect();

        if let (Some(rect_ls), Some(rect_poly)) = (bbox_ls, bbox_poly) {
            // Check for bbox separation along both axes
            let x_separated =
            ...
            if x_separated || y_separated {
                return distance_polygon_polygon_fast(line_string, polygon.exterior());
            }
        }
...
impl<F: GeoFloat> Distance<F, &Polygon<F>, &Polygon<F>> for Euclidean {
    fn distance(&self, polygon_a: &Polygon<F>, polygon_b: &Polygon<F>) -> F {
        ...
        if let (Some(rect_a), Some(rect_b)) = (bbox_a, bbox_b) {
            ...
            if x_separated || y_separated {
                return distance_polygon_polygon_fast(polygon_a.exterior(), polygon_b.exterior());
            }
        }
...
/// Optimized minimum distance calculation for linearly-separable 2D geometries
///
/// # Algorithm Overview
///
...
fn distance_polygon_polygon_fast<F: GeoFloat>(
    linestring_p: &LineString<F>,
    linestring_q: &LineString<F>,
) -> F {
    // Calculate bounding box centroids.
    // Safety: both polygons have already been checked for emptiness by the caller,
    // so bounding_rect() will return Some.
    let bbox_p = linestring_p.bounding_rect().unwrap();
    let bbox_q = linestring_q.bounding_rect().unwrap();
    ...
    // to the polygon bbox centroid connector.
    ...
    let poly_p_coords = &linestring_p.0;
    let poly_q_coords = &linestring_q.0;
    ...
    // Intercepts computed from valid polygon coordinates are never NaN, so unwrap is safe
    ...
    // Step 3: Determine which polygon is "left" (lower bbox centroid intercept value) vs "right"
