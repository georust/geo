...
- `Contains<Point>` / `Contains<Coord>` for `GeometryCollection` now uses the union semantics of the collection, consistent with `relate` and JTS: a point on the shared boundary of two adjacent polygon elements lies in the interior of their union and is contained. Previously the check was member-wise and reported such points as not contained.
  - BREAKING: these impls (and the delegating impls on `Geometry`) now require `T: GeoFloat` instead of `T: GeoNum`, because the evaluation goes through the relate engine. Point containment for integer coordinates remains available on the concrete geometry types.
