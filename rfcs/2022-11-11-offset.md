...
This is a cheap offset algorithim that is suitable for flat coordinate systems
(or if your lat/lon data is near the equator and there is no need for
correctness)
...
- ... maybe some closed shapes like triangle, rect, and the most diffucult is
  polygon and multi polygon. Possibly this algorithim is not suitable for
  polygon, and other operations like th the minkowski sum is more appropriate in
  those cases?
...
- [ ] No checking for zero length input. Invalid results may be caused by
  division by zero.
